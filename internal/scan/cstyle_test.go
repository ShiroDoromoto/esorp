...
func TestCStyleDocNotationAndNestedBlocks(t *testing.T) {
	// Rust / TS を載せる前に、LangSpec が doc 記法とネストを受け止められることを確かめる。
	// 各言語の spec そのものは、その言語を載せるタスクで書く。
	spec := LangSpec{
		Name:        "doc-and-nesting",
		LineComment: "//",
		BlockOpen:   "/*",
		BlockClose:  "*/",
		BlockNests:  true,
	...
	got := comments(CStyle([]byte(src), spec))
	want := []want{
		{line: 1, endLine: 1, col: 1, kind: KindDocLine, text: "/// doc 行"},
		{line: 2, endLine: 2, col: 1, kind: KindDocLine, text: "//! module doc"},
		{line: 3, endLine: 3, col: 1, kind: KindLine, text: "// ただの行"},
		{line: 4, endLine: 4, col: 1, kind: KindDocBlock, text: "/** doc ブロック */"},
		{line: 5, endLine: 5, col: 1, kind: KindBlock, text: "/* 外 /* 内 */ まだ中 */"},
		{line: 6, endLine: 6, col: 1, kind: KindBlock, text: "/**/"},
	}

	if len(got) != len(want) {
		t.Fatalf("コメント数 = %d, want %d\n得たもの: %#v", len(got), len(want), got)
	}
	for i, w := range want {
		g := got[i]
		if g.Kind != w.kind || g.Line != w.line || g.EndLine != w.endLine || g.Col != w.col || g.Text != w.text {
			t.Errorf("comment[%d] = {%v %d-%d:%d %q}, want {%v %d-%d:%d %q}",
				i, g.Kind, g.Line, g.EndLine, g.Col, g.Text, w.kind, w.line, w.endLine, w.col, w.text)
		}
	}
}
