...
# 折り返しが作った当たりかもしれず、原文には直す箇所が無いかもしれない、という断りです。
#
...
rules:
  ...
  - id: internal-ref
    # 追跡番号への参照。ツールは参照の形を知りません——シャープ記号に数字が続く下のパターンは、
    # よくある採番の一例を流し込んだだけです。自分の採番規約に直してください（別の接頭辞を
    # 使っているなら、そう書き換える）。ここを直さないかぎり、このルールは他人の規約を見ています。
    pattern: '#\d+'
    message: |
      追跡番号への参照です。読み手（将来の参加者・外部の読者・次のエージェント）は追跡システムを
      辿れません。番号ではなく、コードだけで読める形で書いてください。
    where:
      # コメントの面だけに当てます。text（コミットメッセージ・PR 本文）を外しているのは、そこが
      # 参照の正しい置き場だからです。コードから追い出した参照の行き先を追いかけて塞ぐと、
      # 行き場が無くなります。
      #
      # 上の syntax: からエントリを消したら、この並びからも消してください。syntax: に無い名前は
      # 設定エラー（exit 2）になります。where.syntax に「これ以外」を書く口はありません。
