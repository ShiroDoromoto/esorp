...
// ならない）。当てる本文は層2 とまったく同じもの（scan.Unwrap で折り返しを畳んだ本文）なので、
// ここで当たった数は、その語彙を rules: に足したときに当たる数そのものになる。ただし層1（器と書式）
	...
	for _, m := range paths {
		...
		for _, c := range comments {
			...
			if re.MatchString(body) {
				t.Hits = append(t.Hits, Hit{Path: m.path, Body: body, Comment: c})
			}
		}
	}
