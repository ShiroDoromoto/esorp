...
ほとんど出なかった句だけが残っています。入れなかったものと、その理由:

| 入れなかった句 | 実測 |
| --- | --- |
| `old` / `before` / `legacy` / `旧` / `previously` | 時間や新旧を指す汎用語は、ほぼ全部が偽陽性（「〜する前に」「古い方の値」は正当な説明） |
| `deprecated` | 316件中245件が Go の `Deprecated:` マーカー＝API の現在の状態であって、履歴ではない |
| `no longer` | 「今そうでない状態」の記述（"closed when no longer needed"）が支配的。安い正規表現では救えない |
| `used to`（素） | 93% が「〜するために使われる」という目的の用法。主語を前に置いて初めて履歴だけを指す |
| `従来`（素） | 4分の3 が「従来どおり／従来挙動」＝今の挙動の説明 |
