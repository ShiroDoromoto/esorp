...
コメントにも当たります）。全部直してから始める必要はありません。**CI は変更分に賭け、既にある一覧は
別に見て削っていきます。**

```sh
esorp check --diff   # 触った分だけを見る（CI とフックが走らせるのはこれ）
esorp check          # ツリー全体 — 読んで削っていくための、素の一覧
```

`check --diff` は、既にある違反を**構造的に**素通しします — 差分に無いものは当たらないからです。
猶予を記録する生成ファイルは要りません（esorp が持つのは、バイナリと `esorp.yaml` だけです。
履歴は git が持っています）。

正当なコメントまで赤くなるなら、それは猶予する話ではなく、ルールを直す話です — 器を `allow` に
列挙するか、`severity: advisory` に置きます（後述）。

**インライン抑制コメント（`// esorp:ignore`）はありません。** 抑制コメント自体が「許可されていない
器のコメント」になって自己矛盾しますし、違反を消す代わりに抑制を足す抜け道になります。例外は
`esorp.yaml` に宣言します（＝一覧として見える状態にする）。
...
コミットのたびにツリー全体を見たいなら `args: [check]` で上書きします。
...
終了コード（`0` / `1` / `2`）と `--format`（text | json）は、ツリーの監査と同じです。
...
rules:
  ...
  - id: internal-ref
    ...
    where:
      syntax: ["!text"]      # text 以外のすべて＝コメントの面だけ
```
...
`where.syntax` が `text` を外しているのは、**コミットメッセージが参照の正しい置き場**だからです。
コードから追い出した参照の行き先がそこなので、追いかけて塞ぐと行き場が無くなります。履歴を止める
`no-history` が `text` にも当たるのと対になります（履歴には行き先がありません）。
...
3. `severity:` で `advisory` にする（報告には出ますが、CI は落としません）
...
```yaml
    where:
      ...
      syntax: ["!text"]                          # 「!」始まりは除外。除外だけなら「それ以外すべて」
      kind: [line, block]                        # 省略時は全 kind
      path: ["**/*.go", "!internal/legacy/**"]   # 省略時は全ファイル。「!」始まりは除外
```

`syntax` と `path` は「`!`」始まりで除外を書けます。**除外はいつも勝つ**ので、正のエントリに当たって
いても除外に当たれば落ちます（並べる順は結果を変えません）。除外だけを並べたときの意味は2つの軸で
違います。`syntax` は「**それ以外すべて**」（値域が `syntax:` のキーと `text` で閉じているので、
「これ以外」が決まります）。`path` は**何も選びません**（glob は開いた空間で、正の glob が0本だと
選ぶ相手がいないため、設定エラーにしています）。

同じ名前を正と除外の両方に書くと、**設定エラー**です。除外がいつも勝つので、その名前は決して選ばれ
ません。

```yaml
    where:
      syntax: [cstyle, "!cstyle"]      # エラー。cstyle に「当てる」と「当てない」を同時に宣言している
      path: ["**/*.go", "!**/*.go"]    # 同上
```

黙って通すと、そのルールはどの面にも当たらないまま、`no violations` と報告されます。検査されていない
のに適合したように見えるので、読み込みの時点で落とします。

`where.syntax` の除外が**面を1つ残らず覆った**ときは、**警告**です。「それ以外すべて」が空になり、
そのルールは何にも当たりません。読み込みは通り（終了コードは動きません）、そのことを標準エラー出力へ
告げます。
...
rules:
  - id: dead-by-exhaustion
    where:
      syntax: ["!cstyle", "!hash", "!text"]   # 警告。面が3つなら、残る面はゼロ
```

エラーではなく警告なのは、この形が「それ以外すべて」という正当な機能の縁にあり、面を足せば自然に
直るからです。落とすほどの確信がありません。同じ判定を `where.path` は持ちません — glob は開いた
空間で、除外が全体を覆うかを一般に判定できません。
...
違反ごとの「強さ」を、違反 id を鍵にした表で持てます。鍵は `disposition:` と同じ id 空間（層1 の
違反 id と、`rules[].id`）です。

```yaml
severity:
  form-paragraphs: advisory   # 疑わしいが、CI は落とさない
  no-history: advisory
```

値は **`enforce`（既定）と `advisory` の2つだけ**です。`esorp check` が非ゼロで終わるのは `enforce`
の違反だけで、`advisory` は報告に出ますが CI を落としません。**書かれていない id は `enforce`** です
（ツールの中に隠れた強度はありません）。

使いどころは、ルールを既存のコードへ導入・強化する場面です。当てた初日は正当なコメントまで赤くなる
ので、そこを `advisory` にして、報告を見ながら直し、直りきったら表から消して `enforce` へ戻します。

**`off` はありません。**「効かせない」は既にある言い方で全部言えます — 層1 の書式は
`headings: allow` / `subject: off`、器は `allow` に列挙する、層2 は `rules:` からエントリを消す。
二つ目の言い方を足せば、同じことが二箇所で言えて、必ず食い違います。

強度は id ごとなので、器ごとには弱められません（`place-not-allowed` を `advisory` にすると、
`leading` の違反も `doc` の違反も一緒に弱まります）。器を1つだけ通したいなら、それは `allow` に
列挙するのが本筋です。
...
| インライン抑制コメント | 自己矛盾するし、抜け道になる。例外は `esorp.yaml` に宣言する |
