...
**ツールは既定の語彙を持ちません。** 文字列マッチは必ず誤検知します。4つのコーパス（Go 標準
ライブラリ 248,920コメント、日本語のプロジェクト2件 8,209、英語のプロジェクト1件 1,335）で
測ったところ、「時間や新旧を指す汎用語」（`old` / `before` / `legacy` / `旧` / `previously` /
`deprecated`）はほぼ全部が偽陽性でした（「〜する前に」「古い方の値」という正当な説明）。
`deprecated` に至っては、その 78% が API の現在の状態を告げるマーカーで、履歴ではありません。

**専用句でも、言い方が広すぎれば同じことが起きます。** 英語の `used to` は 93% が「〜するために
使われる」という目的の用法で、主語（`this` / `we` / …）を前に置いて初めて履歴だけを指しました。
`no longer` は「今そうでない状態」の記述（"closed when no longer needed"）が支配的で、安い正規
表現では救えません。日本語の「以前」も、素で当てると半分以上が偽陽性でした（「v2 以前の形式」＝
互換の境界）。係助詞「は」を伴い、かつ直前が文頭・句読点・空白のときだけが履歴を指したので、上の
パターンはその形に絞っています。**誤検知するガードは例外指定を誘発し、やがてツールごと無視され
ます。** 足すなら「変化を指す専用句」だけに絞り、**足す前に自分のコーパスで測ってください。**
...
実際、esorp 自身のツリーではルール語が30件以上出てきますが、その全部がテストデータと散文で、
当たっているものは1件もありません。
